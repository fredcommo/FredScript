...
rot3D.2 <- function(X, rotation = c("x.axis", "y.axis", "z.axis"), Col = rep("blue", nrow(X)), ptSize = 1.5, Asp = NA,  n.image=16, folder = getwd()){
	...
	for(i in seq(0, 2, len=n.image)){
		...
		plot(X[,1:2], type="n", bty="n", axes = F, xlab = "", ylab = "", xlim = range(-max.d, max.d)*1.25, ylim = range(-max.d, max.d), asp = Asp) #, xlim=range(-2.5, 3), ylim=range(-2, 2))
		t = i*pi
		...
		pcex <- ptSize*(Xp_rot[,3] - mp)/(Mp - mp) + 0.5
