Richard.5PL.v2 <- function(x, y, Plot = F, add.line = F, add.intercept = F, Xlab = "", Ylab = "", Title = ""){
...
# z <- (y - Fb.ini*0.95)/(Fmax.ini*1.05 - Fb.ini*0.95)
...
# Représentations graphiques
	if(Plot){
	plot(y~x, pch = 1, cex = 1.25, col = "royalblue1", xlim = range(newx), ylim = range(yfit), xlab = Xlab, ylab = Ylab)
