...
.computeInformQ <- function(D, a, Df){
...
.computeTraceQ <- function(Data){
  # Returns the trace of the cov. matrix
  acpTest <- prcomp(Data)
  return(sum(acpTest$sdev^2))
}

pcaTraceQ <- function(Data, PCA, Dim = 2:3, weight = 0.25, Plot = TRUE,...){
  require(parallel)
  ...
  tmpTrace <- .computeTraceQ(t(Data))
  a <- Score$aValue[nrow(Score)]
  delta.a <- a/Score$aValue[nrow(Score)-1]
  Score <- rbind(Score, cbind(aValues = a*delta.a, nProbes = nrow(Data), Trace = tmpTrace))
  ...
  model <- .RichardQ(x, y, w = weight, Plot = Plot, add.points = TRUE,
...
pcaSelectQ <- function(pcaScore, p = 0.05){
  # pcaScore: pcaTrace output
...
.RichardQ <- function(x, y, w = 0.25, Plot = FALSE, add.points = FALSE,
