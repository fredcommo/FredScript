...
CAAI.4 <- function(SegTable, Centr, alpha = 1e7, thetaP = 5e-3, thetaQ = 1.2){
	Score <- c()
	for(i in 1:24){
		pArm = paste(i, "p", sep = "")
		qArm = paste(i, "q", sep = "")
		p <- SegTable[which(SegTable$chrom == i & SegTable$loc.start <= Centr[i]), ]
		q <- SegTable[which(SegTable$chrom == i & SegTable$loc.start >= Centr[i]), ]
		npSeg <- nrow(p)
		nqSeg <- nrow(q)
	...
	Score$caai <- as.numeric(as.vector(Score$caai))
