...
rot3D.3 <- function(X, rotation = c("x.axis", "y.axis", "z.axis"), Col = rep("blue", nrow(X)), ptSize = 1.5, Asp = NA,  n.image=16, folder = getwd()){
	...
	for(i in seq(0, 2, len=n.image)){
		...
		t = i*pi
		max.d <- sqrt(max(X[,1])^2 + max(X[,3])^2 + max(X[,3])^2)
		...
		pcex <- ptSize*(Xp_rot[,3] - mp)/(Mp - mp) + 0.75
		ord <- order(pcex, decreasing = F)

		plot(Xp_rot[,1:2], type = "n", bty = "n", axes = F, xlab = "", ylab = "", asp = Asp) #xlim = range(-max.d, max.d)*1.25, ylim = range(-max.d, max.d), xlim=range(-2.5, 3), ylim=range(-2, 2))
		points(Xp_rot[ord,1], Xp_rot[ord,2], pch = 19, col = pCol, cex = pcex[ord])
		points(Xp_rot[ord,1], Xp_rot[ord,2], pch = 19, col = "grey90", cex = pcex[ord]/5)
