Dose.Resp4P <- function(Dose, Resp, Prop=FALSE, Blank=0, T0=0, wi=T, w.coef=2, add=F, print.legend=TRUE, ExcludeMinMax=FALSE, unit="µM"){
...
# Avec nlm
	...
	# initialisation des valeurs
		bottom.ini = min(y, na.rm=T); bottom.ini
		top.ini = max(y, na.rm=T); top.ini
		xmid.ini = (max(x)+min(x))/2; xmid.ini
		z <- y/(max(y, na.rm=T)*1.05)
	...
	# Représentations graphiques
		plot(z~x, pch = 20, cex = 1.25, col = "blue", xlim=range(min(newX), max(newX)), ylim = range(0, 1))
