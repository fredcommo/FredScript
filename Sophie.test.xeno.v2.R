# Function
Sophie.test.xeno.v2 <- function(data, Test, Ref, Drug.test=c("SU","ZD"), expandlo = 1.5, expandhi = 3, leg.pos=c(-1.7, -1.9)){
...
# End Part2
		...
		boxplot(rlm.results$estimate~rlm.results$Drug:rlm.results$Time,  border=NA,
			ylim=range(min.y, max.y), names=rep(c("Placebo",Drug.test), 2), 
