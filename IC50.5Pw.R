IC50.5Pw <- function(dose, rep, sd, W.coef=0.5, unit="nM", Title="", Plot.sd = TRUE, AddIC = TRUE, Add = FALSE, ptCol="indianred3", lCol="royalblue4"){
...
# Avec nlm
	...
	# Fonction sce (somme carré résidus) avec pondérations
		sce.5P <- function(param, X, yobs, Weights, W.coef) {
			...
			W.test <- nlm(f = wOptim, p = W.coef, yobs = yobs, ytheo = ytheo)
			best.W <- W.test$estimate[1]
			residus <- yobs - ytheo
			Weights <- (1/(residus^2))^(best.W)
			return(sum(Weights*(yobs - ytheo)^2))
			}

	# Fonction optimisation des poids
	wOptim <- function(W.coef, yobs, ytheo){
			residus <- yobs - ytheo
	...
	# Représentations graphiques
		...
		if(AddIC){
			...
			text(c(x[1], x[1]), c(0.3, 0.15), pos=4, labels = c(legend1, legend2), font=4, cex=1.4, col="royalblue4")
			}
