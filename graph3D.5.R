graph3D.5<-function(X,class1=NULL,class2=NULL,size=2, log.base=4, bold=TRUE,title=NULL)
...
# Begin
{
	...
	# Convert classes to factors if they are not.
		if (!is.null(class1) & is.numeric(class1))				
			class1<-as.factor(class1)
		if (!is.null(class2) & is.numeric(class2))
	...
	# Define color values
		...
		col.palette<-c("royalblue2", "indianred1", "seagreen3", "goldenrod2", "blue3", "purple1", "burlywood2", "firebrick1", "green4", "deeppink1", "deepskyblue2")	# 1 = "black" is excluded	
		{
	...
	# Define points values
		...
		{
		if(is.null(class1) & is.null(class2)) points=19
		else{
	...
	# draw the 3D graph
		x <- x[order(z, decreasing = F)]
		y <- y[order(z, decreasing = F)]
		color <- color[order(z, decreasing = F)]
		points <- points[order(z, decreasing = F)]
		d1 <- d1[order(z, decreasing = F)]
		z <- z[order(z, decreasing = F)]
