graph3D.9.2<-function(X, class1 = NULL, class2 = NULL, size = 0.5, Cols = NULL, log.base = 2, bold = TRUE, title = NULL, add.legend = T, Theta = 0, Phi = 0, Z = 0,...)
...
# Begin
{
	...
	# Define points values
		...
		Cex <- -log(d1/(max(d1)*1.05))

	# draw the 3D graph
		X <- cbind(x, y, z)
		X <- X[order(Cex, decreasing = T),]
		x <- X[,1]
		y <- X[,2]
		z <- X[,3]
		color <- color[order(Cex, decreasing = T)]
		points <- points[order(Cex, decreasing = T)]
		d1 <- d1[order(Cex, decreasing = T)]
		Cex <- sort(Cex, decreasing = T)
		
		if(Theta+Phi+Z == 0) cloud(z~x*y, pch = points, col = color, cex = Cex*size, 
						xlab = name.x, ylab = name.y, zlab = name.z, main = title,...)
	
		else 			cloud(z~x*y, pch = points, col = color, cex = Cex*size, 
