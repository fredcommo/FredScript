BoxPoints5 <- function(x, y, densit=FALSE, add.stat = c("none", "add.med", "add.mean"), add.sem = T, ColType = c("C", "G", "P"), 
				Cols = NA, Pch = NA, disp=0.05, leg.pos = "auto",...){	#pt.size=1, title=NULL, Xlab = "", Ylab = "Values", Ylim = NA, 
...
require(affy) 	# for tukey.biweight()
	...
	points(y ~ X, col = col.palette, pch = ifelse(!is.na(Pch), Pch, 19),...)	# cex = pt.size, pch = c(2:(nlevels(x)+1))[x], pch = 19,
