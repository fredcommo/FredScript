graph3D.10 <- function(X, class1 = NULL, class2 = NULL, size = 2, Cols = NULL, log.base = 2, bold = TRUE, title = NULL, add.legend = T, Theta = 0, Phi = 0, Z = 0,...)
...
# Begin
{
	...
	# draw the 3D graph
		Cex <- -log(d1/(max(d1)*1.05), base = log.base)*size
		Cex <- ifelse(Cex < 0.5, 0.5, Cex)
		ord <- order(d1, decreasing = F)
		x <- x[ord]
		y <- y[ord]
		z <- z[ord]
		color <- color[ord]
		...
		else 			cloud(z~x*y, pch = points, col = color, cex = Cex, 
