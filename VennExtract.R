VennExtract<-function (results, Contr, include = "both") 
{
    	x <- as.matrix(results)
	...
	which(xlist[[1]]==0 & xlist[[2]]==1 & xlist[[3]]==1)->list1_2
	which(xlist[[1]]==1 & xlist[[2]]==0 & xlist[[3]]==1)->list1_3
	which(xlist[[1]]==1 & xlist[[2]]==1 & xlist[[3]]==0)->list2_3
	which(xlist[[1]]==1 & xlist[[2]]==1 & xlist[[3]]==1)->list1_2_3

    															#list(grpA=list1,grpB=list2,grpC=list3)->discrim

	counts <- as.vector(table(xlist))
    	list(Table = cbind(outcomes, Counts = counts),
		list1 = list1,
		list2 = list2,
		list3 = list3,
		list1_2 = list1_2,
		list1_3 = list1_3,
		list2_3 = list2_3,
		list1_2_3 = list1_2_3)	#Grps=discrim
}
