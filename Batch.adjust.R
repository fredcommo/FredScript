Batch.adjust <- function(Eset, info, qnorm = F, Center = T, Scale = T){
...
require(preprocessCore)
	...
	sEset <- scale(as.data.frame(Eset), center = Center, scale = Scale)
	...
	for (i in 1:length(batch)){
		...
		tmp <- apply(tmp, 1, scale, center = Center, scale = Scale)
