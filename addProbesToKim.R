...
.generateSubGrp <- function(Mvector, Svector, p, grps = grps,
                            subGrps = NULL, nSubGrp = 2, minP = 0.3, maxP = 0.8){
  if(is.na(grps)) stop('Please provide a vector of groups')
  if(!is.factor(grps)) grps <- as.factor(grps)
  if(is.null(subGrps)){
    subGrps = c(); k = 1
    for(l in levels(grps)){
      n <- sum(grps == l)
      tmp <- LETTERS[k:(k+nSubGrp-1)][rbinom(n, nSubGrp - 1, 0.5)+1]
      k = k + nSubGrp
      subGrps <- c(subGrps, tmp)
    }
    subGrps <- factor(subGrps)
  }
  output <- .generateGrp(Mvector, Svector, length(grps), p,
                         grps = subGrps, minP = minP, maxP = maxP)
  return(list(Data = output, subGrps = subGrps))
}

################################################################
saveDir <- '/Users/fredcommo/Documents/MyProjects/Simulations/'
...
ns1 <- ns(M, df = 3)
lm1 <- lm(log10(S) ~ ns1)
png(file = paste0(saveDir, 'real_SdevMeans_distrib.png'), width = 800, height = 800)
...
png(file = paste0(saveDir, 'real_SdevMeans_boxplot.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
...
png(file = paste0(saveDir, 'random_SdevMeans_boxplot.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
...
png(file = paste0(saveDir, 'simulated_SdevMeans_distrib.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
...
png(file = paste0(saveDir, 'pca_simulProbesLocation.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
...
png(file = paste0(saveDir, 'pca_realProbesLocation.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
plot(pcaProbes$x[,2:3], cex = 0.1,
     col = ifelse(grepl('white', rownames(eset)), 'red', ifelse(grepl('signif', rownames(eset)), 'blue', 'grey')))
...
png(file = paste0(saveDir, 'informationCurve.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
...
png(file = paste0(saveDir, 'pca_filteredProbesLocation.png'), width = 800, height = 800)
par(cex.main = 2.5, cex.lab = 1.75, cex.axis = 1.25)
plot(pcaProbes$x[,2:3], cex = 0.1,
     col = ifelse(grepl('white', rownames(eset)), 'red', ifelse(grepl('signif', rownames(eset)), 'blue', 'grey')))
points(pcaProbes$x[bestFull,2:3], cex = 0.25, col = 'cyan')
...
png(file = paste0(saveDir, 'heatmap_filteredProbes_Significant.png'), width = 800, height = 800)
heatmap.3(Values,
          # Colv = NA,
          # dendrogram = "column",
...
png(file = paste0(saveDir, 'heatmap_filteredProbes_070.png'), width = 800, height = 800)
heatmap.3(Values,
          # Colv = NA,
          # dendrogram = "column",
          scale = "column", Method = "ward",
...
png(file = paste0(saveDir, 'heatmap_filteredProbes_070_clusters.png'), width = 800, height = 800)
heatmap.3(Values,
          # Colv = NA,
          # dendrogram = "column",
          scale = "column", Method = "ward",
