MultiBox3.scale<-function(class, val, xlab="class", ylab="values", nchar=10, title=NULL, add.legend=TRUE, legcex=0.75)
...

{
	...
	if (add.legend){
		if (p<=20)	{	
			...
			p.leg <- floor(p/3)
			legend("bottomleft",colnames(val.scale)[1:p.leg],fill=rainbow(p,1)[1:p.leg],cex=legcex, bty="n")
			legend("bottom",colnames(val.scale)[(p.leg+1):(p.leg*2)],fill=rainbow(p,1)[(p.leg+1):(p.leg*2)],cex=legcex, bty="n")
			legend("bottomright",colnames(val.scale)[(p.leg*2+1):p],fill=rainbow(p,1)[(p.leg*2+1):p],cex=legcex, bty="n")
			}
		}
}
