# Function
Sophie.test <- function(data, Test, Ref, Drug.test=c("SU","ZD")){
	n <- length(Drug.test)
	par(mfrow=c(1,n))
	All.Res <- c()

	for(D in 1:n){
		index.D <- which(Drug== Drug.test[D] | Drug== "Placebo")	# Valeurs pour ZD & Placebo
			...
			for (t in c(2, 1, 3)){
				...
				if (levels(drug)[d]=="Placebo") d.dose <- "0"
					...
					for (dos in 1:length(d.dose)){
						...
						rlm.results<-rbind(rlm.results, c(time, levels(drug)[d], d.dose[dos], estim, s))
						}
				}
			}

		rlm.results <- as.data.frame(rlm.results)
		colnames(rlm.results) <- c("Time", "Drug", "Dose", "estimate", "error")
	...
	return(list(All.Res = All.Res, summary(aov2<-aov(estimate~Drug+Dose+Time, data=All.Res))))
}
