graph3D.9.1 <-function(X, class1 = NULL, class2 = NULL, size = 0.5, Cols = NULL, log.base = 2, bold = TRUE, title = NULL, add.legend = T, Theta = 0, Phi = 0, Z = 0,...)
...
# Begin
{
	...
	# draw the 3D graph
		# Cex <- -log(d1/(max(d1)), base = log.base)*size
		Cex <- -1/log(d1/(max(d1)*1.05), base = log.base)*size
		ordX <- cbind(x, y, z)[order(Cex, decreasing = F),]
		x <- ordX[,1]; y <- ordX[,2]; z <- ordX[,3]
		color <- color[order(Cex, decreasing = F)]
		points <- points[order(Cex, decreasing = F)]

		plot.new()
		if(Theta+Phi+Z == 0) cloud(z ~ x*y, pch = points, col = color, cex = Cex, 
		...
		#		}
		#	}		
		# return(cbind.data.frame(ordX, d1, Cex))
} 
